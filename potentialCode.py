...
def quiverplot(X, Y, U, V, title):
    ...
    """
    CP = plt.quiver(X, Y, U, V, linewidth = 1, color = 'b', units = 'x', width = 0.022)
    plt.axes().set_aspect('equal', 'datalim')
    plt.title(title)
    plt.xlabel("X")
    plt.ylabel("Y")
    qv = cStringIO.StringIO()
    plt.savefig(qv, format = "png")
    plot = "data:image/png;base64," + qv.getvalue().encode("base64").strip()
    plt.clf()
    qv.close
    return plot
...
def getCaption(objs):
    """
    Returns the caption for the figure
    """
    i = 0
    name = ""
    for obj in objs:
...
def plotsifi(objs, p1=(-5, -5), p2 = (5, 5), fi = False):
    ...
    for obj in objs:
        Zsi = Zsi + obj.si(X, Y)
    return contourplot(X, Y, Zsi, "Streamlines in the flowfield")
    if fi:
        Zfi = np.zeros(X.shape)
        Zfi = Zfi + obj.fi(X, Y)
        return contourplot(X, Y, Zfi, "Potential lines in the flowfield")
...
def plotquiver(objs, p1=(-5, -5), p2 = (5, 5)):
    ...
    for obj in objs:
        u += obj.getu(X, Y)
        v += obj.getv(X, Y)
    return quiverplot(X, Y, u, v, "Quiver plot for velocities")
